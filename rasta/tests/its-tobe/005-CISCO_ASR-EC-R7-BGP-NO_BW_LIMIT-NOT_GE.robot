...
Task: Configure "PE_Endpoint" edge ports for testing
    Step "1": Create PE_Endpoint in "prepare-test" state
    Step "2": ActionChecks "PreTestChecks-1-for-PE-Endpoint"
    Step "3": Action Healthcheck
    Step "4": ChangeToStateTest
...
Task: Activate "PE_Endpoint" BGP
    ...
    Step "4": Action Healthcheck
    Step "5": ActionChecks "PostConfigChecks-1-for-PE-Endpoint"
    Step "6": Action Healthcheck
    Step "7": ChangeToStateReady
    Step "8": Action Healthcheck
...
Step "${step}": Create PE_Endpoint in "prepare-test" state
    [Documentation]   Creates a service instance on NSO via REST
    [Tags]  #service
        via NSO REST API configure device "${nso}" at URI "${itsrfsuri}" using "PATCH" with "json" payload "${createInStatePrepareTestPayload}"
...
Step "${step}": ChangeToStateTest
    [Documentation]   Modifies a service instance on NSO via REST
    [Tags]  #service
        via NSO REST API configure device "${nso}" at URI "${modifyToStateuri}" using "PATCH" with "json" payload "${modifyToStateTestPayload}"
